...
export class NinBvnPortalProvider extends IKycProvider {
    ...
    async _request(endpoint, method = "POST", body = null) {
        ...
        try {
            Logger.info(`[NinBvnPortal] Sending ${method} request to: ${endpoint}`);
            ...
            if (response.ok && result.status === "success") {
                ...
                Logger.warn(`[NinBvnPortal] API error for ${endpoint}`, { message: result.message || response.statusText, code: result.code });
                return {
                    success: false,
            ...
            Logger.error(`[NinBvnPortal] Request Error [${endpoint}]`, error);
            return {
                success: false,
